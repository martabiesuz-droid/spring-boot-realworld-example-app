...
public class ProfileQueryServiceMockTest {
  ...
  @Mock private UserReadService mockUserReadService;
  ...
  public void should_check_following_when_current_user_present() {
    ...
    when(mockUserReadService.findByUsername(anyString())).thenReturn(userData);
    when(mockUserRelationshipQueryService.isUserFollowing(anyString(), anyString()))
        .thenReturn(true);
    ...
    verify(mockUserRelationshipQueryService, times(1))
        .isUserFollowing(currentUser.getId(), userData.getId());
    verify(mockUserReadService, times(1)).findByUsername("testuser");
  }
}
